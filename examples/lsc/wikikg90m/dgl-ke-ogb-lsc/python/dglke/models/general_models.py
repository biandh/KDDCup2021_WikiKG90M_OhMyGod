...
class KEModel(object):
    ...
    def predict_neg_score(self, pos_g, neg_g, to_device=None, gpu_id=-1, trace=False, neg_deg_sample=False):
        ...
        if neg_g.neg_head:
            ...
            neg_score = self.tail_neg_score(head, rel, neg_tail, num_chunks, chunk_size, neg_sample_size)
        ...
        if mode == 'h,r->t':
            if self.encoder_model_name == 'roberta':
                ...
                head = self.transform_net.embed_entity(torch.cat(
                    [self.entity_feat(query[:, 0], gpu_id, False), self.entity_emb(query[:, 0], gpu_id, False)], -1))
        ...
        if self.encoder_model_name == 'shallow_net':
            ...
            pos_g.ndata['emb'] = self.transform_net.embed_entity(
                torch.cat([
                    ...
                    self.entity_emb(pos_g.ndata['id'], gpu_id, True)  # 这里需要注意要不要更新!!!!!!!!
                ], -1))
