...
class ComplExScore(nn.Module):
    ...
    def create_neg(self, neg_head):
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size, neg_sample_size):
                hidden_dim = heads.shape[1]
                real_head = heads[..., :hidden_dim // 2]
                img_head = heads[..., hidden_dim // 2:]
                real_rel = relations[..., :hidden_dim // 2]
                img_rel = relations[..., hidden_dim // 2:]

                real = real_head * real_rel - img_head * img_rel
                imag = real_head * img_rel + img_head * real_rel
...
class RotatEScore(nn.Module):
    ...
    def edge_func(self, edges):
        ...
        # print('re_tail.shape: %s, im_tail.shape: %s' % (re_tail.shape, im_tail.shape))
        # print('re_rel.shape: %s, im_rel.shape: %s' % (re_rel.shape, im_rel.shape))
        # print('re_head.shape: %s, re_rel.shape: %s, im_head: %s, im_rel: %s' % (re_head.shape, re_rel.shape, im_head.shape, im_rel.shape))
        ...
        print('edge_func spend time:', time.time() - t)
    ...
    def create_neg(self, neg_head):
        ...
        if neg_head:
            def fn(heads, relations, tails, num_chunks, chunk_size, neg_sample_size):
                ...
                emb_real = tails[..., :hidden_dim // 2]
                emb_imag = tails[..., hidden_dim // 2:]
                # print('create_neg head spend time1: ', time.time() - t)
                phase_rel = relations / (emb_init / np.pi)
                # print('create_neg head spend time2: ', time.time() - t)
                rel_real, rel_imag = th.cos(phase_rel), th.sin(phase_rel)
                # print('create_neg head spend time3: ', time.time() - t)
                real = emb_real * rel_real + emb_imag * rel_imag
                # print('create_neg head spend time4: ', time.time() - t)
                imag = -emb_real * rel_imag + emb_imag * rel_real
                # print('create_neg head spend time5: ', time.time() - t)
                emb_complex = th.cat((real, imag), dim=-1)
                # print('create_neg head spend time6: ', time.time() - t)
                tmp = emb_complex.reshape(num_chunks, chunk_size, 1, hidden_dim)
                # print('create_neg head spend time7: ', time.time() - t)
                heads = heads.reshape(num_chunks, 1, neg_sample_size, hidden_dim)
                # print('create_neg head spend time8: ', time.time() - t)
                score = tmp - heads
                print('create_neg head spend time8: ', time.time() - t)
                score = th.stack([score[..., :hidden_dim // 2],
                                  score[..., hidden_dim // 2:]], dim=-1)

                print('create_neg head spend time9: ', time.time() - t)
                score = score.norm(dim=-1)

                print('create_neg head spend time: ', time.time() - t)
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size, neg_sample_size):
                ...
                print('create_neg tail spend time: ', time.time() - t)
                return gamma - score.sum(-1)
